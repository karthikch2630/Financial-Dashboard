...
interface TransactionTableProps {
  transactions?: Transaction[]; // Data passed from Pagination logic
  isFullPage?: boolean;  // If true, doesn't slice the list
}
export const TransactionTable = ({ transactions: propTransactions, isFullPage = false }: TransactionTableProps) => {
  ...
  // If propTransactions exists (Pagination), use it.
  // Otherwise, sort store data by newest first.
  let displayTransactions = propTransactions || [...storeTransactions].sort(
    (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
  );

  // ✅ If on Dashboard (not full page) and no props passed, show only 7
                  ...
                  {role === "admin" && (
                    <td className="py-3.5 pr-2 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button className="p-1.5 bg-gray-900/50 border border-gray-800 rounded-xl text-gray-500 hover:text-emerald-400 hover:border-emerald-500/50 transition-all">
